...
def classify(file_path):
    ...
    # Chuyển ảnh về RGB nếu ảnh có chỉ 1 kênh (grayscale)
    if image.mode != 'RGB':
    ...
    # Chuyển ảnh thành mảng numpy và mở rộng chiều của nó
    image = numpy.expand_dims(image, axis=0)
    image = numpy.array(image)

    # Kiểm tra kích thước của ảnh
    print(image.shape)  # Đảm bảo đầu ra là (1, 30, 30, 3) cho ảnh RGB
